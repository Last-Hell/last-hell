...
void ProcHelperGuildStashKeep(CNetMsg::SP& msg)
{
	...
	std::vector<CItem*> pVector = pc->m_removingItemList;
	std::vector<CItem*>::iterator begin = pVector.begin();
	for (; begin != pVector.end(); ++begin)
		LogGuildStash(*begin, (*begin)->getItemCount(), GUILD_STASH_KEEP_LOG, pc);
	pc->RemoveItemFromList(false);
