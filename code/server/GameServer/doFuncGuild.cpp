...
// 		SEND_Q( rmsg, ch->m_desc );
	...
	if (packet->keepMoney > 0)
	{
		...
		ch->m_inventory.decreaseMoneyNotSend(packet->keepMoney);
	}
...

	{
		...
		for (int i = 0; i < packet->listCount; ++i)
		{
			...
			pItem->clone(*newItem);
			newItem->setItemCount(packet->list[i].count);
			ch->AddItemToRemovingList(newItem);
			ch->m_inventory.decreaseItemCount(pItem, packet->list[i].count);
		}
